<template>
      <v-form v-model="valid" ref="BrandForm">
        <v-layout row>
          <v-flex xs4>
            <span style="font-size: 20px; color: #444">幻灯片图片</span>
          </v-flex>
          <v-flex>
        ...
        <v-text-field v-model="brand.letter" label="幻灯片ID" required mask="A" :rules="[v => !!v || '幻灯片ID不能为空']"></v-text-field>
        <v-layout class="pt-3" >
          <v-spacer></v-spacer>
          <!--<v-btn color="blue darken-1" flat @click="clear">清空</v-btn>-->
          <v-btn color="blue darken-1" flat @click="submit">确定</v-btn>
        </v-layout>
      </v-form>
</template>
